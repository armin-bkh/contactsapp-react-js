...
const ContactList = ({ contacts, onSearch }) => {
    ...
    if(error) {
        returnValue = (
            ...
            <h1 className={`text-yellow-400 font-header font-bold`}>No contact's</h1>
        </section>    
        )
    }
    if(contacts && !error){
    ...
    return ( 
        ...
        <header className={`border-gray-300 flex blocks items-center justify-between px-3 py-1`}>
            ...
            <div className={`relative mt-2`}>
                <input className={`inp border-yellow-400 font-medium w-full px-3 py-1 rounded-md bg-transparent outline-none`} placeholder="search..." type="text" value={search} onChange={searchHandler} />
